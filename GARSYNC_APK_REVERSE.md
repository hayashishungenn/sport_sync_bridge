...
本地活动源已接入现有 Garmin / Strava 上传流程；原始文件存入 `.data/local_imports/`，数据库记录摘要和指纹。42 份训练计划模板覆盖六种语言，33 个 FIT 课表模板也随项目提供。健康数据通过用户提供的 CSV 导入；该实现不登录或抓取 APK 内的健康平台账号。已将 HR-TSS、CTL、ATL、TSB 计算加入本地活动库，并在 AI 活动分析上下文中展示 FIT 自带 TSS。
